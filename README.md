...
* 대부분의 스트리밍 서비스는 노래를 플레이리스트에 담은 후에는 그룹화를 할 수가 없다. 미리 그룹을 만들어 놓은 후에 노래를 플레이리스트에 담을 때 그룹화를 해야한다. 기존 서비스의 이러한 부분이 불편함 점으로 다가와 동적으로 그룹화 할 수 있는 서비를 만들고자 했다.
...
* #### ✅ 웹플레이어 - 반복재생, 셔플, 재생/정지, 다음곡/다시듣기, 재생목록 제거, 장르별그룹화, Drag & Drop으로 노래 순서 변경 가능
